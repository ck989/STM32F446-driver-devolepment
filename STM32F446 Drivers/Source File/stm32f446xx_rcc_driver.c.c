...
/******************************** Configuration of CR1******************************************/
	...
	}else if (pUSARTHandle->USART_PinConfig.USART_Mode == USART_MODE_ONLY_TX)
	{
...
/******************************** Configuration of CR2******************************************/
	...
	tempreg |= pUSARTHandle->USART_PinConfig.USART_NoOfStopBits << USART_CR2_STOP;
...
/******************************** Configuration of CR3******************************************/
	...
	}else if (pUSARTHandle->USART_PinConfig.USART_HWFlowControl == USART_HW_FLOW_CTRL_RTS)
	{
		//Implement the code to enable RTS flow control
		tempreg |= (1 << USART_CR3_RTSE);

	}else if (pUSARTHandle->USART_PinConfig.USART_HWFlowControl == USART_HW_FLOW_CTRL_CTS_RTS)
	{
...
/********************************************************************************************
 * @fn                            -    IRQ Interrupt configure USART clock
 *
 * @breif                         -
 *
 * @param[in]                     -
...
/********************************************************************************************
 * @fn                            -    IRQ Priority USART configure clock
 *
 * @breif                         -
 *
 * @param[in]                     -
   ...
   //Loop over until "Len" number of bytes are transferred
	for(uint32_t i = 0 ; i < Len; i++)
	{
		...
		if(pUSARTHandle->USART_PinConfig.USART_WordLength == USART_WORDLEN_9BITS)
		{
			...
			if(pUSARTHandle->USART_PinConfig.USART_ParityControl == USART_CR1_PCE)
			{
		...
		if(pUSARTHandle->TxBusyState == USART_BUSY_IN_TX)
		{
			...
			if(pUSARTHandle->TxLen > 0)
			{
				...
				if(pUSARTHandle->USART_PinConfig.USART_WordLength == USART_WORDLEN_9BITS)
				{
					...
					pUSARTHandle->pUSARTx->DR = (uint16_t*)((int)pdata & 0x01FF);
		...
		if(pUSARTHandle->RxBusyState == USART_BUSY_IN_RX)
		{
			...
			if(pUSARTHandle->RxLen > 0)
			{
				...
				if(pUSARTHandle->USART_PinConfig.USART_WordLength == USART_WORDLEN_9BITS)
				{
					...
					{
						//Parity is used. so, 8bits will be of user data and 1 bit is parity
						 *((uint8_t*)pUSARTHandle->pRxBuffer) = (pUSARTHandle->pUSARTx->DR  & (uint8_t)0xFF);
 ...
 */
void USART_SetBaudRate(USART_RegDef_t *pUSARTx, uint8_t BaudRate)
{
